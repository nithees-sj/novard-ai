...
const ProjectsInlineView = () => {
  ...
  const fetchCareers = useCallback(async () => {
    try {
      const response = await axios.get(`${apiUrl}/api/projects/careerIds`);
  ...
  const handleSelectCareer = async (careerId) => {
    ...
    setError(null);
    try {
      const response = await axios.get(`${apiUrl}/api/projects/${careerId}`);
  ...
  const handleAddNewCareer = async () => {
    ...
    setError(null);
    try {
      await axios.post(`${apiUrl}/api/projects/process`, { career: career, count });
      setNewCareer('');
      await fetchCareers();
  ...
  const handleDeleteCareer = async (careerId) => {
    ...
    setError(null);
    try {
      await axios.delete(`${apiUrl}/api/projects/delete/${careerId}`);
...
export default ProjectsInlineView;
